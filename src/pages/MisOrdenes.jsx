...
export default function MisOrdenes() {
  ...
  const cargarOrdenes = async () => {
    ...
    try {
      ...
      setOrdenes(
        lista.map((o) => ({
          ...o,
          empleado: o.empleado_id ? mapa.get(o.empleado_id) || null : null,
        }))
      )
    } catch (e) {
  ...
  const cancelarOrden = async (id) => {
    ...
    if (!orden || orden.estado !== 'pendiente')
      return setError('Esta orden ya no puede ser cancelada.')
    if (
      !window.confirm(
    ...
    setCancelandoId(id)
    setError('')
    const { error: e } = await supabase.rpc('cancelar_orden_pc_store', {
      p_orden_id: id,
    ...
    <main className="pc-page">
      <div className="pc-container">
        ...
        <div className="pc-page-heading">
          <div>
            <span className="pc-kicker">Mi cuenta</span>
            <h1>Mis órdenes</h1>
            <p>Consulta tus compras, datos de entrega, costos y estado.</p>
          </div>
        </div>
        ...
        >
          <input
            ...
            placeholder="Buscar orden, producto, teléfono o dirección..."
              ...
              >
                ...
                >
                  <div>
                    <div className="pc-kicker">
                      Orden #{orden.id} · {orden.numero_orden || 'Sin número'}
                    </div>
                    ...
                    <div style={{ marginTop: 5, color: '#666', fontSize: 13 }}>
                      {orden.created_at ? new Date(orden.created_at).toLocaleString() : '-'}
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 16 }}>
                  {ESTADOS.map(([valor, texto]) => (
                    <span
                      key={valor}
                      className={`pc-status pc-status-${valor}`}
                      style={{
                        padding: '7px 11px',
                        fontWeight: 700,
                        opacity: estado === valor ? 1 : 0.45,
                        border:
                          estado === valor ? '2px solid currentColor' : '1px solid currentColor',
                      }}
                    >
                      {texto}
                    </span>
                ...
                >
                  <section style={{ background: '#fafafa', borderRadius: 12, padding: 14 }}>
                    ...
                    <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.7 }}>
                      {empleado ? (
                        <div>
                          <div>
                            <b>Empleado encargado:</b> {empleado.nombre || ''}{' '}
                            {empleado.apellido || ''}
                          </div>
                          {empleado.teléfono && (
                            <div>
                              <b>Teléfono empleado:</b> {empleado.teléfono}
                            </div>
                          )}
                        </div>
                      ) : (
                        <div style={{ color: '#777' }}>
                          Empleado encargado: Pendiente de asignación
                        </div>
                      )}
                      <div>
                        <b>Receptor:</b> {orden.nombre_cliente || '-'}{' '}
                        {orden.apellido_cliente || ''}
                      </div>
                      <div>
                        <b>Teléfono:</b> {orden.teléfono_contacto || '-'}
                      </div>
                      <div>
                        <b>Dirección:</b> {orden.dirección_envío || '-'}
                      </div>
                      <div>
                        <b>Ciudad:</b> {orden.ciudad_envío || '-'}
                      </div>
                      {orden.referencia && (
                        <div>
                          <b>Referencia:</b> {orden.referencia}
                        </div>
                      )}
                      {orden.notas && (
                        <div>
                  ...
                  <section style={{ background: '#fafafa', borderRadius: 12, padding: 14 }}>
                    ...
                    <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.8 }}>
                      <div>
                        Subtotal: <b>L {subtotal.toFixed(2)}</b>
                      </div>
                      <div>
                        Envío: <b>L {envio.toFixed(2)}</b>
                      </div>
                      <div>
                        Método de pago: <b>{orden.método_pago || orden.metodo_pago || '-'}</b>
                      </div>
                    </div>
                  </section>
                  <section style={{ background: '#fafafa', borderRadius: 12, padding: 14 }}>
                    ...
                    <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.7 }}>
                      {(orden.items || []).map((item, i) => (
                        <div
                          key={`${orden.id}-${item.producto_id || i}`}
                          style={{
                            display: 'flex',
                            justifyContent: 'space-between',
                            gap: 10,
                        ...
                        >
                          ...
                          <b>L {Number(item.subtotal || 0).toFixed(2)}</b>
                        </div>
                      ))}
                    </div>
                  </section>
                ...
                {estado === 'cancelada' && orden.motivo_cancelacion && (
                  <div
                    style={{
                      background: '#fef2f2',
                  ...
                  >
                    <b>Motivo de cancelación:</b> {orden.motivo_cancelacion}
                  </div>
                )}
                {puedeCancelar && (
                  <div style={{ marginTop: 16 }}>
                    <button
                      ...
                      disabled={cancelandoId === orden.id}
                      onClick={() => cancelarOrden(orden.id)}
                    >
                      {cancelandoId === orden.id ? 'Cancelando...' : 'Cancelar orden'}
                    </button>
                  </div>
                )}
              </article>
