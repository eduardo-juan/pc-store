...
export default function MisOrdenes() {
  ...
  const confirmarRecepcion = async (id) => {
    ...
    if (!window.confirm('¿Confirmas que recibiste esta orden?')) {
      return
    }
  ...
  const ordenesFiltradas = useMemo(() => {
    ...
    return ordenes.filter((orden) => {
      ...
      if (
        filtro === 'activas' &&
        !['pendiente', 'pagada', 'enviada'].includes(estado)
      ) {
        return false
      }
      ...
      if (
        fecha &&
        new Date(orden.created_at).toLocaleDateString('en-CA') !==
          fecha
      ) {
        return false
      }
      ...
      <div className="pc-container">
        ...
        <div className="pc-page-heading">
          <div>
            ...
            <h1>Mis órdenes</h1>
            <p>
              Consulta tus compras, datos de entrega, costos y estado.
            </p>
          </div>
        </div>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div>
                        Subtotal:{' '}
                        <b>L {subtotal.toFixed(2)}</b>
                      </div>

                      <div>
                        Envío:{' '}
                        <b>L {envio.toFixed(2)}</b>
                      </div>
                ...
                {estado === 'enviada' && (
                  <div style={{ marginTop: 16 }}>
                    <button
                      ...
                      disabled={confirmandoId === orden.id}
                      onClick={() =>
                        confirmarRecepcion(orden.id)
                      }
                    >
                      {confirmandoId === orden.id
                        ? 'Confirmando...'
                        : 'Confirmar recepción'}
                    </button>
                  </div>
