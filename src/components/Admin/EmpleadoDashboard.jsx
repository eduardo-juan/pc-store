...
export default function EmpleadoDashboard() {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    const [ordenesResult, disponiblesResult, comisionesResult, configResult] =
      await Promise.all([
        supabase
          .from("ordenes")
          .select("id,total,estado,empleado_id,created_at")
    ...
    if (ordenesResult.error) {
      setError(ordenesResult.error.message);
    }

    if (disponiblesResult.error) {
      setError(disponiblesResult.error.message);
    }

    if (comisionesResult.error) {
      setError(comisionesResult.error.message);
    }
    ...
    setMetricas({
      ...
      ventas: ordenes
        ...
        .reduce((suma, orden) => suma + Number(orden.total || 0), 0),

      pendientes: ordenes.filter((orden) => orden.estado === "pendiente")
        .length,

      pagadas: ordenes.filter((orden) => orden.estado === "pagada").length,

      enviadas: ordenes.filter((orden) => orden.estado === "enviada").length,

      entregadas: ordenes.filter((orden) => orden.estado === "entregada")
        .length,

      activas: ordenes.filter((orden) =>
        ["pendiente", "pagada", "enviada"].includes(orden.estado),
      ).length,

      comisiones: (comisionesResult.data || []).reduce(
        (suma, comision) => suma + Number(comision.monto || 0),
        0,
      ),
    });
  ...
  const aceptarOrden = async (ordenId) => {
    ...
    const { error: rpcError } = await supabase.rpc("aceptar_orden_empleado", {
      p_orden_id: ordenId,
    });

    if (rpcError) {
      setError(rpcError.message || "No se pudo aceptar la orden.");
    }
  ...
  const opciones = [
    ...
    },
    {
      ...
      ruta: "/admin/empleado/comisiones",
    },
  ];
      ...
      <div className="pc-container">
        ...
        <div className="pc-page-heading">
          <div>
            ...
            <h1 style={{ color: "#171717" }}>
              Hola, {perfil?.nombre || "Empleado"}
            </h1>

            <p style={{ color: "#171717" }}>
              Las nuevas órdenes aparecen aquí automáticamente. Elige las que
              quieras gestionar.
            </p>
          </div>
        </div>
        ...
        <div className="pc-metrics-grid">
          <article className="pc-card pc-metric">
            <span style={{ color: "#171717" }}>Órdenes activas</span>

            <strong style={{ color: "#171717" }}>
              {metricas.activas} / {maxOrdenes}
            </strong>
          </article>

          <article className="pc-card pc-metric">
            <span style={{ color: "#171717" }}>Órdenes gestionadas</span>

            <strong style={{ color: "#171717" }}>{metricas.ordenes}</strong>
          </article>

          <article className="pc-card pc-metric">
            <span style={{ color: "#171717" }}>Entregadas</span>

            <strong style={{ color: "#171717" }}>{metricas.entregadas}</strong>
          </article>

          <article className="pc-card pc-metric">
            <span style={{ color: "#171717" }}>Comisiones</span>

            <strong style={{ color: "#171717" }}>
              L {metricas.comisiones.toFixed(2)}
            </strong>
          </article>
        </div>
        ...
        >
          ...
          >
            <div>
              ...
              <p className="pc-muted" style={{ margin: "5px 0 0" }}>
                Todos los empleados activos reciben estas órdenes. El primero
                que la acepta la gestiona.
              </p>
            </div>
          ...
          {ordenesDisponibles.length === 0 ? (
            <div className="pc-muted" style={{ padding: "20px 0" }}>
              No hay órdenes nuevas disponibles en este momento.
            </div>
          ) : (
            <div
              style={{
