    ...
    <main className="pc-page">
      <div className="pc-container">
        ...
        <div className="pc-page-heading">
          <div>
            <span className="pc-kicker" style={{ color: '#171717' }}>Panel de empleado</span>
        ...
        <section className="pc-card" style={{ padding: 20, marginBottom: 30 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
            <div>
              <h2 style={{ margin: 0, color: '#171717' }}><Bell size={20} style={{ verticalAlign: 'middle', marginRight: 7 }} />Nuevas órdenes</h2>
              <p className="pc-muted" style={{ margin: '5px 0 0' }}>Todos los empleados activos reciben estas órdenes. El primero que la acepta la gestiona.</p>
            </div>
              ...
              {ordenesDisponibles.map((orden) => (
                <article key={orden.id} className="pc-card" style={{ padding: 16, border: '1px solid rgba(0,0,0,.08)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                    <div>
                      <strong style={{ color: '#171717' }}>{orden.numero_orden || `Orden #${orden.id}`}</strong>
                      <p style={{ margin: '5px 0', color: '#171717' }}>{orden.nombre_cliente} {orden.apellido_cliente} · {orden.ciudad_envío}</p>
                      <p className="pc-muted" style={{ margin: 0 }}>{orden.dirección_envío}</p>
                      {orden.referencia && <p className="pc-muted" style={{ margin: '5px 0 0' }}>Referencia: {orden.referencia}</p>}
                    </div>
