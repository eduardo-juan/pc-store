...
export default function RegistroEmpleado() {
  ...
  const validar = () => {
    ...
    Object.entries(form).forEach(([campo, valor]) => {
      if (!String(valor ?? '').trim()) nuevos[campo] = true
    })
  ...
  const guardar = async (e) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('crear_empleado', {
        body: {
          ...form,
  ...
  const clase = (campo) => `pc-input${errores[campo] ? ' pc-input-error' : ''}`
  ...
  const Campo = ({ campo, label, ...props }) => (
    <label style={{ display: 'block' }}>
      ...
      />
      {errores[campo] && (
        <small style={{ display: 'block', marginTop: 5, color: '#dc2626' }}>
          Completa este campo.
        </small>
      )}
    </label>
    ...
    <main className="pc-page">
      <div className="pc-container">
        ...
        <form className="pc-card pc-profile-form" onSubmit={guardar} style={{ maxWidth: 900, margin: '0 auto' }}>
          {errorGeneral && (
            <div style={{ padding: 14, marginBottom: 20, border: '1px solid #fecaca', background: '#fef2f2', color: '#b91c1c', borderRadius: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 22 }}>
            <UserPlus size={22} />
            <h2 style={{ margin: 0 }}>Datos del empleado</h2>
          </div>
          ...
          <div className="pc-form-grid">
            ...
            <label style={{ display: 'block' }}>
              <span style={{ display: 'block', marginBottom: 7, fontWeight: 600 }}>
          ...
          <label style={{ display: 'block', marginTop: 18 }}>
            <span style={{ display: 'block', marginBottom: 7, fontWeight: 600, color: errores.dirección ? '#dc2626' : '#171717' }}>
              Dirección <span style={{ color: '#dc2626' }}>*</span>
            </span>
            <textarea
              className={`pc-textarea${errores.dirección ? ' pc-input-error' : ''}`}
              rows="3"
          ...
          <label style={{ display: 'block', marginTop: 18 }}>
            <span style={{ display: 'block', marginBottom: 7, fontWeight: 600, color: errores.password ? '#dc2626' : '#171717' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              />
              <button type="button" onClick={() => setMostrarPassword((v) => !v)} title={mostrarPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'} aria-label={mostrarPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', border: 'none', background: 'transparent', cursor: 'pointer', padding: 6 }}>
          ...
          <p style={{ marginTop: 18, color: '#64748b', fontSize: 14 }}>
            Los campos marcados con <span style={{ color: '#dc2626' }}>*</span> son obligatorios. La contraseña debe tener al menos 6 caracteres.
          </p>

          <div style={{ display: 'flex', gap: 10, marginTop: 22, flexWrap: 'wrap' }}>
            <button type="submit" className="pc-btn pc-btn-primary" disabled={guardando}>
              {guardando ? 'Creando cuenta...' : 'Crear cuenta de empleado'}
            </button>
            <button type="button" className="pc-btn pc-btn-light" onClick={() => navigate('/admin/empleados')} disabled={guardando}>
              Cancelar
            </button>
          </div>
        </form>
