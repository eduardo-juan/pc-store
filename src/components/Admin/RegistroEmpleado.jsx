...
function Campo({
  ...
  return (
    <label>
      ...
      <input
        className={`pc-input${error ? ' pc-empleado-input-error' : ''}`}
        type={type}
        inputMode={inputMode}
        maxLength={maxLength}
...
export default function RegistroEmpleado() {
  ...
  const validar = () => {
    ...
    Object.entries(form).forEach(([c, v]) => {
      if (!String(v ?? '').trim()) e[c] = true
    })
    if (form.email && !/^\S+@\S+\.\S+$/.test(form.email)) e.email = true
    if (form.password && form.password.length < 6) e.password = true
    if (form.teléfono && !/^\d{4}-\d{4}$/.test(form.teléfono)) e.teléfono = true
    if (form.dni && !/^\d{4}-\d{4}-\d{5}$/.test(form.dni)) e.dni = true
    setErrores(e)
  ...
  const guardar = async (e) => {
    ...
    setGuardando(true)
    try {
      const { data, error } = await supabase.functions.invoke('crear_empleado_api', {
  ...
  return (
    <main className="pc-page">
      <div className="pc-container">
        ...
        >
          {errorGeneral && (
            <div style={{ padding: 14, marginBottom: 20, color: '#b91c1c' }}>{errorGeneral}</div>
          )}
          ...
          <label className="pc-empleado-field" style={{ display: 'block', marginTop: 18 }}>
            <span className="pc-required-label">
              Dirección <b className="pc-required">*</b>
            </span>
            <textarea
              className={`pc-textarea${errores.dirección ? ' pc-empleado-textarea-error' : ''}`}
              rows="3"
              maxLength={150}
              value={form.dirección}
              onChange={(e) => cambio('dirección', e)}
            />
            {errores.dirección && <small>Completa este campo.</small>}
          </label>
          <label className="pc-empleado-field" style={{ display: 'block', marginTop: 18 }}>
            <span className="pc-required-label">
            ...
            <div className="pc-password-wrap">
              <input
                className={`pc-input${errores.password ? ' pc-empleado-input-error' : ''}`}
            ...
            {errores.password && <small>Completa este campo.</small>}
          </label>
          <label className="pc-empleado-field" style={{ display: 'block', marginTop: 18 }}>
            <span className="pc-required-label">
