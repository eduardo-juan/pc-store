...
export default function GestionEmpleados() {
  ...
  const quitarEmpleado = async (id) => {
    if (!window.confirm('¿Quieres quitar el rol de empleado y devolver esta cuenta a cliente?'))
      return
    setError('')
    const { error } = await supabase
      .from('usuarios')
  ...
  const empleados = usuarios
    ...
    .filter((u) => {
      ...
      return `${u.nombre || ''} ${u.apellido || ''} ${u.email || ''} ${u.ciudad || ''}`
        .toLowerCase()
      ...
      <div className="pc-container">
        ...
        <section className="pc-card" style={{ padding: 20, marginBottom: 30 }}>
          ...
          <p className="pc-muted">
            Estos valores se aplican globalmente. El límite se valida en el servidor al aceptar una
            orden.
          </p>
