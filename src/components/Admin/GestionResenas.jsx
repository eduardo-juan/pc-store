...
export default function GestionResenas() {
  ...
  async function cargar() {
    ...
    const { data, error } = await supabase
      .from("reseñas")
      .select(
        "id, producto_id, usuario_id, calificación, comentario, útil_count, created_at, productos(nombre), usuarios(nombre, email)",
      )
    ...
    <main className="pc-page">
      <div className="pc-container">
        ...
        <div className="pc-page-heading">
          <div>
            <span className="pc-kicker">Administración</span>
            <h1>Reseñas</h1>
            <p>Modera y consulta las valoraciones de los clientes.</p>
          </div>
        </div>
        ...
        {cargando ? (
          ...
          <div className="pc-empty-small">No hay reseñas.</div>
        ) : (
              ...
              <tbody>
                {resenas.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td>
                      {r.usuarios?.nombre || r.usuarios?.email || "Usuario"}
                    </td>
                    <td>
                      <span className="pc-review-stars">
                        {[1, 2, 3, 4, 5].map((n) => (
                          <Star
                            key={n}
                            size={16}
