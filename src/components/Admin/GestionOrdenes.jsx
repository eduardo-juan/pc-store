...
const limpiarBusqueda = (valor) =>
  valor.replace(/[^\p{L}\p{N}\s@._-]/gu, '').slice(0, 100)
...
export default function GestionOrdenes() {
  ...
  const cargarOrdenes = async () => {
    ...
    if (esAdmin) {
      ...
      if (ids.length) {
        const { data: productos, error: proveedoresError } = await supabase
  ...
  const ordenesFiltradas = useMemo(() => {
    ...
    return ordenes.filter((orden) => {
      ...
      if (filtro === 'activas' && !ESTADOS_ACTIVOS.includes(estado)) {
        return false
      }
  ...
  const totalPaginas = Math.max(
    1,
    Math.ceil(ordenesFiltradas.length / ORDENES_POR_PAGINA),
  )
    ...
    <main className="pc-page">
      <div className="pc-container">
        ...
        {error && (
          <div
            className="pc-card"
            style={{ padding: 14, marginBottom: 18, color: '#dc2626' }}
          >
            {error}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {cargando ? (
            ...
            ordenesPagina.map((orden) => {
              const estado = orden.estado || 'pendiente'
              const estilo = estiloEstado[estado] || estiloEstado.pendiente
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <div>
                          Método de pago:{' '}
                          <b>{orden['método_pago'] || orden.metodo_pago || '-'}</b>
                        </div>
                      </div>
                    </section>
                      ...
                      {ESTADOS.filter(([valor]) =>
                        ...
                        return (
                          <button
                            ...
                            style={{
                              border: `1px solid ${
                                estado === valor ? estilo.border : '#ddd'
                              }`,
                              background:
                                estado === valor ? estilo.background : '#fff',
                              color: estado === valor ? estilo.color : '#444',
                              borderRadius: 9,
                              padding: '8px 13px',
                              fontWeight: 700,
                              cursor: deshabilitado ? 'not-allowed' : 'pointer',
                              opacity:
                                deshabilitado && estado !== valor ? 0.45 : 1,
                            }}
                          >
                            {texto}
                          </button>
          ...
          >
            ...
            >
              ...
              <p style={{ color: '#555', lineHeight: 1.5 }}>
                Escribe el motivo que se mostrará al cliente. Esto también
                quedará guardado en el historial de la orden.
              </p>
