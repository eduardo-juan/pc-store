...
export default function GestionOrdenes() {
  ...
  const [ordenes, setOrdenes] = useState([]),
    ...
    [cancelacion, setCancelacion] = useState(null),
  ...
  const cargarOrdenes = async () => {
    ...
    if (esAdmin) {
      const ids = [
        ...new Set(
          cargadas.flatMap((o) => (o.items || []).map((i) => i.producto_id).filter(Boolean))
        ),
      ]
      if (ids.length) {
        ...
        const mapa = {}
        ;(pd || []).forEach((p) => {
          if (!mapa[p.producto_id]) mapa[p.producto_id] = p
        })
        setProveedores(mapa)
      } else setProveedores({})
    } else setProveedores({})
  ...
  const cambiarEstado = async (id, nuevoEstado) => {
    ...
    if (actual === 'pagada')
      return setError('Una orden pagada está cerrada y ya no puede modificarse.')
  ...
  const confirmarCancelacion = async () => {
    ...
    setProcesando(true)
    setError('')
    const { error: rpcError } = await supabase.rpc('cancelar_orden_pc_store', {
  ...
  const totalPaginas = Math.max(1, Math.ceil(ordenesFiltradas.length / ORDENES_POR_PAGINA)),
    ...
    ordenesPagina = ordenesFiltradas.slice(
  ...
  return (
    <main className="pc-page">
      <div className="pc-container">
        ...
        {error && (
          <div className="pc-card" style={{ padding: 14, marginBottom: 18, color: '#dc2626' }}>
            {error}
          </div>
        )}
        <div
        ...
        <div style={{ display: 'grid', gap: 16 }}>
          {cargando ? (
            <div className="pc-card" style={{ padding: 30, textAlign: 'center' }}>
              Cargando órdenes...
            </div>
            ...
            ordenesPagina.map((orden) => {
              const estado = orden.estado || 'pendiente',
                s = estiloEstado[estado] || estiloEstado.pendiente,
              ...
              return (
                ...
                >
                  ...
                  >
                    <section style={{ background: '#fafafa', borderRadius: 12, padding: 14 }}>
                      ...
                      <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.7 }}>
                        <div>
                          <b>Tel:</b> {orden.teléfono_contacto || '-'}
                        </div>
                        <div>
                    ...
                    <section style={{ background: '#fafafa', borderRadius: 12, padding: 14 }}>
                      ...
                      <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.8 }}>
                        ...
                        <div>
                          Método de pago: <b>{orden.metodo_pago || '-'}</b>
                        </div>
                      </div>
                    </section>
                    <section style={{ background: '#fafafa', borderRadius: 12, padding: 14 }}>
                  ...
                  <div style={{ borderTop: '1px solid rgba(0,0,0,.08)', paddingTop: 16 }}>
                      ...
                      {ESTADOS.map(([valor, texto]) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => cambiarEstado(orden.id, valor)}
                            style={{
                              border: `1px solid ${estado === valor ? s.border : '#ddd'}`,
                              background: estado === valor ? s.background : '#fff',
                              color: estado === valor ? s.color : '#444',
                              borderRadius: 9,
                              padding: '8px 13px',
                              fontWeight: 700,
                              cursor: deshabilitado ? 'not-allowed' : 'pointer',
                              opacity: deshabilitado && estado !== valor ? 0.45 : 1,
                            }}
                          >
                            {texto}
                          </button>
                    ...
                    {estado === 'pagada' && (
                      <div style={{ marginTop: 7, fontSize: 12, color: '#666' }}>
          ...
          >
            <div className="pc-card" style={{ width: 'min(560px,100%)', padding: 24 }}>
              ...
              <p style={{ color: '#555', lineHeight: 1.5 }}>
                Escribe el motivo que se mostrará al cliente. Esto también quedará guardado en el
