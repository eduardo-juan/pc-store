...
export default function HistorialComisionesEmpleado() {
  ...
  const cargarComisiones = async () => {
    ...
    const { data, error: consultaError } = await supabase
      ...
      .select(
        'id,orden_id,porcentaje,base,monto,created_at,ordenes(numero_orden,total)',
      )
  ...
  const totalMes = diasDelMes.reduce(
    (suma, dia) => suma + dia.total,
    0,
  )

  const entregasMes = diasDelMes.reduce(
    (suma, dia) => suma + dia.cantidad,
    0,
  )
    ...
    <main className="pc-page">
      <div className="pc-container">
        ...
        <div className="pc-admin-header">
          ...
          <p>
            Historial de comisiones generadas por cada entrega
            completada.
          </p>
        </div>
