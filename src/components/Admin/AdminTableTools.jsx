...
const obtenerMapaIds = async () => {
  ...
  if (ruta === '/admin/productos') {
    const { data } = await supabase.from('productos').select('id,nombre,marca').order('id')
    return { tipo: 'productos', data: data || [] }
  }
  if (ruta === '/admin/usuarios' || ruta === '/admin/empleados') {
    const { data } = await supabase.from('usuarios').select('id,nombre,apellido,email').order('id')
    return { tipo: 'usuarios', data: data || [] }
  }
  if (ruta === '/admin/inventario') {
    const { data } = await supabase.from('inventario_historial').select('id,producto_id,created_at,productos(nombre)').order('created_at', { ascending: false }).limit(50)
    return { tipo: 'inventario', data: data || [] }
  }
  return null
}
...
const aplicarIdsTabla = (table, mapa) => {
  ...
  const filas = Array.from(table.querySelectorAll('tbody tr')).filter((fila) => fila.querySelectorAll('td').length)
  filas.forEach((fila) => {
    ...
    if (mapa.tipo === 'productos') {
      registro = mapa.data.find((item) => texto.includes((item.nombre || '').toLowerCase()) && (!item.marca || texto.includes((item.marca || '').toLowerCase())))
    } else if (mapa.tipo === 'usuarios') {
      registro = mapa.data.find((item) => {
        const email = (item.email || '').toLowerCase()
        const nombre = `${item.nombre || ''} ${item.apellido || ''}`.trim().toLowerCase()
        return (email && texto.includes(email)) || (nombre && texto.includes(nombre))
      })
    }
...
const aplicarIdsInventario = (mapa) => {
  ...
  document.querySelectorAll('.pc-card').forEach((card) => {
    ...
    const registro = mapa.data.find((item) => {
      ...
      const fecha = item.created_at ? new Date(item.created_at).toLocaleDateString('es-HN') : ''
      return !fecha || texto.includes(fecha)
    })
    if (!registro) return
    const etiqueta = document.createElement('div')
    etiqueta.textContent = `ID: ${registro.id}`
    etiqueta.style.cssText = 'font-size:11px;opacity:.55;margin-bottom:4px;font-weight:600;'
...
const instalar = (wrapper, mapa) => {
  ...
  limpiar.addEventListener('click', () => { buscador.value = ''; fecha.value = ''; pagina = 1; aplicar() })
...
export default function AdminTableTools() {
  useEffect(() => {
    let mapa = null
    const cargarMapa = async () => { mapa = await obtenerMapaIds() }
    cargarMapa()

    const escanear = () => {
      if (!esRutaAdmin()) return
      aplicarIdsInventario(mapa)
      document.querySelectorAll('.pc-table-wrapper').forEach((wrapper) => instalar(wrapper, mapa))
    }
