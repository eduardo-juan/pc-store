...
const aplicarIdsTabla = (table, mapa) => {
  ...
  ).filter((fila) => fila.querySelectorAll('td').length)
  ...
  filas.forEach((fila) => {
    ...
    if (mapa.tipo === 'productos') {
      registro = mapa.data.find(
        (item) =>
          ...
          (!item.marca ||
            texto.includes((item.marca || '').toLowerCase()))
      )
    ...
    td.textContent =
      registro?.id != null ? String(registro.id) : '—'
...
const instalar = (wrapper, mapa) => {
  ...
  const aplicar = () => {
    ...
    const termino = limpiarBusqueda(buscador.value)
      .trim()
      .toLowerCase()
    ...
    const visibles = new Set(
      filtradas.slice(inicio, inicio + PAGE_SIZE)
    )
