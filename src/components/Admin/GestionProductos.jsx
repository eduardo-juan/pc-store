...
import { subirImagen, eliminarImagen, obtenerRutaStorage } from '../../services/storageService'
import { Plus, Save, X, Pencil, Trash2, Truck, Image as ImageIcon, Tag, Star } from 'lucide-react'
...
const obtenerRutasImagenesPropias = (producto) => {
  ...
  if (Array.isArray(producto?.imágenes_adicionales)) urls.push(...producto.imágenes_adicionales)
  return [...new Set(urls.map((url) => obtenerRutaStorage(BUCKET_PRODUCTOS, url)).filter(Boolean))]
}

const limpiarImagenes = async (producto) => {
  for (const ruta of obtenerRutasImagenesPropias(producto)) await eliminarImagen(BUCKET_PRODUCTOS, ruta)
}
...
export default function GestionProductos() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const descuentoNumero = precioDescuento === '' ? null : parseFloat(precioDescuento)
      if (!Number.isFinite(precioNumero) || precioNumero <= 0) throw new Error('El precio debe ser mayor que 0.')
      if (descuentoNumero !== null && (!Number.isFinite(descuentoNumero) || descuentoNumero <= 0 || descuentoNumero >= precioNumero)) {
      ...
      if (imagenes.length > 0) {
        ...
        for (let i = 0; i < imagenes.length; i += 1) {
          const resultadoSubida = await subirImagen(BUCKET_PRODUCTOS, imagenes[i], `${nombre}-${Date.now()}-${i + 1}`, `productos/${nombreCategoria}`)
      ...
      if (editandoId) resultado = await actualizarProducto(editandoId, datosProducto)
      else {
      ...
      if (productoAnterior && imagenes.length > 0) await limpiarImagenes(productoAnterior)
      ...
      if (esAdmin && productoId) {
        if (dropshippingActivo && proveedor.trim() && urlProveedor.trim()) {
          ...
          const proveedorResultado = await guardarProveedorProducto(productoId, {
            proveedor: proveedor.trim(), url_compra: urlValida.toString(), costo: parseFloat(costoProveedor || '0'), activo: true,
          })
  ...
  const resetFormulario = () => {
    setNombre(''); setDescripcion(''); setPrecio(''); setPrecioDescuento(''); setDestacado(false); setStock('')
    setMarca(''); setModelo(''); setCategoriaId(''); setImagenes([]); setEditandoId(null)
    setProveedor(''); setUrlProveedor(''); setCostoProveedor(''); setDropshippingActivo(false); setMostrarFormulario(false)
  }
  ...
  const handleEliminar = async (id) => {
    if (!esAdmin) { setError('No tienes permiso para eliminar productos.'); return }
    if (!window.confirm('¿Estás seguro de que deseas eliminar este producto?')) return
    setError('')
    const producto = productos.find((item) => Number(item.id) === Number(id))
    const resultado = await eliminarProducto(id)
    if (resultado.success) { await limpiarImagenes(producto); alert('Producto eliminado'); await cargarProductos() }
    else setError(resultado.error)
  }
      ...
      <div className="pc-container">
          ...
          <div className="pc-card" style={{ padding: 22, marginBottom: 24 }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                <button type="submit" disabled={cargando} className="pc-btn pc-btn-primary" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Save size={17} />{cargando ? 'Guardando...' : 'Guardar'}</button>
                <button type="button" className="pc-btn pc-btn-light" onClick={resetFormulario} style={{ display: 'flex', alignItems: 'center', gap: 8 }}><X size={17} />Cancelar</button>
              </div>
            </form>
          </div>
        )}
        ...
        <div className="pc-card pc-table-wrapper">
          <table className="pc-table">
            <thead><tr><th>Nombre</th><th>Categoría</th><th>Marca</th><th>Precio</th><th>Oferta</th><th>Destacado</th><th>Stock</th><th>Acciones</th></tr></thead>
            <tbody>{productos.map((producto) => {
              ...
              return <tr key={producto.id}>
                <td>{producto.nombre}</td><td>{producto.categorias?.nombre || 'Sin categoría'}</td><td>{producto.marca}</td>
